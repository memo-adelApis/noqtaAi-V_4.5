...
import { getInvoicesWithPendingInstallments } from '@/app/actions/ownerDashboardActions';
import InstallmentActions from '@/components/owner/InstallmentActions';
...
export default async function InstallmentsPage() {
  ...
  const result = await getInvoicesWithPendingInstallments();
  ...
  const getInstallmentStatusBadge = (installment) => {
    ...
    if (daysDiff < 0) {
      return (
        <span className="inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs font-medium text-white bg-red-500">
      ...
      <div className="space-y-8">
        ...
        <div>
          <h1 className="text-3xl font-bold flex items-center gap-3">
            ...
            إدارة الأقساط المتبقية
          </h1>
          <p className="text-gray-400 mt-2">
            متابعة وإدارة الأقساط المستحقة والمتأخرة
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          ...
          <div className="bg-gray-900 p-6 rounded-xl border border-gray-800">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-gray-400 text-sm">إجمالي المبلغ المتبقي</p>
                <p className="text-2xl font-bold text-green-400">{formatCurrency(stats.totalPendingAmount)}</p>
              </div>
          ...
          <div className="bg-gray-900 p-6 rounded-xl border border-gray-800">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-yellow-400">{stats.dueSoonInvoices}</p>
              </div>
        ...
        <div className="space-y-6">
          {invoices.length === 0 ? (
            <div className="bg-gray-900 rounded-xl border border-gray-800 p-12 text-center">
              ...
              <p className="text-gray-400">لا توجد أقساط متبقية حالياً</p>
            </div>
          ) : (
                  ...
                  <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                    <div>
                      ...
                      <div className="flex flex-wrap items-center gap-4 text-sm text-gray-400">
                        <span>الفرع: {invoice.branchId?.name || 'غير محدد'}</span>
