...
import { createSupplier, updateSupplier, deleteSupplier } from '@/app/actions/supplierActions';
import { useRouter } from 'next/navigation';
import toast, { Toaster } from 'react-hot-toast';
import { Plus, Edit2, Trash2, Phone, MapPin, Truck, AlertTriangle } from 'lucide-react'; // (تغيير الأيقونة إلى Truck)
import { z } from "zod";
...
function SupplierFormModal({ isOpen, onClose, supplierToEdit, onSave }) {
    ...
    const handleSubmit = async (e) => {
        ...
        setIsLoading(true);
        const action = isEditMode ? updateSupplier : createSupplier;
...
function DeleteSupplierAlert({ isOpen, onClose, supplierToDelete, onConfirm }) {
    ...
    const handleConfirm = async () => {
        ...
        const result = await deleteSupplier(supplierToDelete._id);
        toast.dismiss(toastId);
...
export default function SupplierClientUI({ initialSuppliers }) {
    ...
    const handleSave = () => {
        ...
        router.refresh(); // إعادة جلب البيانات من السيرفر
    };
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="bg-white shadow-lg rounded-lg overflow-hidden">
                <div className="p-4">
                    <h2 className="text-xl font-semibold">الموردون الحاليون ({initialSuppliers.length})</h2>
                </div>
